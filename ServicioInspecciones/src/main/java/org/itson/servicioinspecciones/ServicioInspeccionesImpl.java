...
import org.itson.catalogodefectos.DefectoResponse;
...
@Override
    ...
    public Uni<ReporteResponse> registrarReporte(NuevoReporteRequest request) {
        return Uni.combine().all().unis(
                productosClient.verificarYObtenerProducto(
                        ProductoRequest.newBuilder().setSku(request.getSkuProducto()).build())
                        .onFailure().transform(t -> new io.grpc.StatusRuntimeException(
                                io.grpc.Status.NOT_FOUND.withDescription("Producto no encontrado"))),
                defectosClient.obtenerDefecto(
                        DefectoRequest.newBuilder().setCodigoDefecto(request.getCodigoDefecto()).build())
                        .onFailure().transform(t -> new io.grpc.StatusRuntimeException(
                                io.grpc.Status.NOT_FOUND.withDescription("Defecto no encontrado")))
        ).asTuple().onItem().transformToUni(tuple -> {
 
            DefectoResponse defecto = tuple.getItem2(); // <-- guardamos el defecto
            ...
            publicador.publicarDefecto(entity, defecto);
    ...
    public Multi<ReporteResponse> obtenerReportes(ReporteRequest request) {
        ...
        switch (request.getFiltro()) {
            case PRODUCTO:  query = ReporteEntity.find("skuProducto",    request.getSkuProducto());    break;
            case LOTE:      query = ReporteEntity.find("lote",           request.getLoteProducto());   break;
            case INSPECTOR: query = ReporteEntity.find("nombreInspector",request.getNombreInspector()); break;
            case DEFECTO:   query = ReporteEntity.find("codigoDefecto",  request.getTipoDefecto());    break;
            case TODOS:
            default:        query = ReporteEntity.findAll(); break;
        }
